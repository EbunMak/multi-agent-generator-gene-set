...
def grade_abstracts(state, llm_name):
    ...
    infile = f"abstracts/gene_annotated_abstracts/{safe_name}"
...
def generate(state, llm_name):
    ...
    txt_fallback = f"{out_dir}/{safe_name}_raw.txt"
    ...
    try:
        ...
        if not generation:
            print("⚠️ Empty or invalid JSON response, trying to clean model output...")
            cleaned_output = clean_model_output(raw_output)
            generation = safe_json_loads(cleaned_output)

        # If still invalid, save raw text fallback
        if not generation:
            with open(txt_fallback, "w") as f:
                f.write(raw_output)
            print(f"⚠️ Saved raw model output to {txt_fallback}")
            generation = []

    except Exception as e:
        print(f"❌ Error during generation for {safe_name}: {e}")
        # Save whatever partial raw output we have
        try:
            with open(txt_fallback, "w") as f:
                f.write(str(e))
            print(f"⚠️ Exception logged to {txt_fallback}")
        except:
            print("⚠️ Could not save raw text fallback.")
        generation = []

    # Save to JSON file if possible
    try:
        with open(outfile, "w") as f:
            json.dump(generation, f, indent=2)
        print(f"✅ Saved generation to {outfile}")
    except Exception as e:
        print(f"❌ Failed to save generation JSON for {safe_name}: {e}")
        # Save raw text instead
        with open(txt_fallback, "w") as f:
            f.write(raw_output)
        print(f"⚠️ Saved invalid JSON output as plain text to {txt_fallback}")

    # Remove phenotype abstracts after processing
    filtered_abstract_file = f"abstracts/gene_annotated_abstracts/{safe_name}__filtered_{llm_name}.json"
...
def create_control_flow():
    ...
    # workflow.add_node("gen_qwen",   lambda s: generate(s, "qwen3:32b"))
    ...
    # workflow.set_entry_point("grade_ds")
    # workflow.set_entry_point("grade_qwen")
    workflow.set_entry_point("grade_llama")
