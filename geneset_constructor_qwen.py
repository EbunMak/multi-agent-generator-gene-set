...
processed_file = "processed_gene_sets_qwen.txt"
...
def merge_extracted_and_verified(
    ...
    ### Only phenotypes appearing in *both* extracted and verified
...
def main():
    ...
    extracted_dir = "out/phenotype_generations/qwen3:32b"
    verified_dir = "out/phenotype_checks/qwen3:32b"
    out_dir = "out/genesets/qwen"
    ...
    # --- inside main() after loading extracted & verified ---
    ### Filter phenotypes: must exist in BOTH extracted AND verified
    ...
    symbols_gmt = os.path.join(out_dir, "genesets_symbols_qwen.gmt")
    entrez_gmt = os.path.join(out_dir, "genesets_entrez_qwen.gmt")
    unmapped = build_gmts_from_merged(merged, symbols_gmt, entrez_gmt)

    # 4) HTML
    html_out = os.path.join(out_dir, "phenotype_gene_summary_qwen.html")
    save_html_summary(merged, html_out)

    # 5) unmapped
    unmapped_out = os.path.join(out_dir, "unmapped_genes_qwen.json")
