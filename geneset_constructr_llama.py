...
def main():
    ...
    extracted_dir = "out/phenotype_generations/llama3.1:8b"
    verified_dir = "out/phenotype_checks/llama3.1:8b"
    out_dir = "out/genesets/llama"
    ...
    symbols_gmt = os.path.join(out_dir, "genesets_symbols_llama.gmt")
    entrez_gmt = os.path.join(out_dir, "genesets_entrez_llama.gmt")
    unmapped = build_gmts_from_merged(merged, symbols_gmt, entrez_gmt)

    # 4) HTML
    html_out = os.path.join(out_dir, "phenotype_gene_summary_llama.html")
    save_html_summary(merged, html_out)

    # 5) unmapped
    unmapped_out = os.path.join(out_dir, "unmapped_genes_llama.json")
